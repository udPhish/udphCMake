...
macro(target_general_setup TARGET_NAME)
    ...
    message("${TARGET_NAME} ${${TARGET_NAME}_VERSION} ${${PROJECT_NAME}_VERSION}")
...
function(check_target TARGET_NAME)
    ...
    message("t ${TARGET_NAME}")
    message("a ${ALIASED_NAME}")
    message("v ${${ALIASED_NAME}_VERSION}")
    if(NOT ALIASED_NAME)
        set(ALIASED_NAME ${TARGET_NAME})
    endif()
    if(${ARGC} GREATER 1)
        if("${${ALIASED_NAME}_VERSION}" VERSION_LESS "${ARGV1}")
            message(FATAL_ERROR "Target ${TARGET_NAME} (${${ALIASED_NAME}_VERSION}) does not meet the minimum version requirement (${ARGV1}).")
        endif()
    endif()
    if(${ARGC} GREATER 2)
        if("${${ALIASED_NAME}_VERSION}" VERSION_GREATER "${ARGV2}")
            message(FATAL_ERROR "Target ${TARGET_NAME} (${${ALIASED_NAME}_VERSION}) does not meet the maximum version requirement (${ARGV2}).")
        endif()
    endif()
endfunction()
